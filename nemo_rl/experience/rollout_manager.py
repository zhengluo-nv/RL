...
class _FinalizedGroupDropped(RuntimeError):
    """Finalizer policy deliberately rejected a completed prompt group."""
...
class AsyncNemoGymRolloutImpl:
    ...
    async def _stream_rows(
        ...
        Args:
            ...
            on_completion: Optional callback invoked once when each row arrives.
...
class RolloutManager:
    ...
    async def generate_and_push(
        ...
        *,
        ...
        Args:
            ...
            recovery_group_id: Existing durable token-capture lineage reservation.
                Callers create it atomically with dataloader advancement; it must
                belong to ``input_sample`` and have the same ``target_step``.
        ...
        if self._finalizer is None and recovery_group_id is not None:
            raise ValueError(
        ...
        while infra_attempts < policy.max_infra_attempts:
            ...
            if self._finalizer is None:
                ...
                if recovery_group_id is None:
                    ...
                    if recovery_group.prompt_id != str(input_sample["idx"]):
                        raise ValueError(
                            "pre-reserved recovery group belongs to a different "
                            f"prompt: group={recovery_group.prompt_id!r}, "
                            f"input={str(input_sample['idx'])!r}"
                        )
                    if recovery_group.target_step != target_step:
                        raise ValueError(
                            "pre-reserved recovery group target step does not match "
                            f"its dispatch: group={recovery_group.target_step!r}, "
                    ...
                    if statuses != {RolloutAttemptStatus.RESERVED}:
                        ...
                        if not statuses <= recoverable_statuses:
                            raise ValueError(
                                f"recovery group {recovery_group_id!r} has "
                                f"unsupported retry states="
                                f"{sorted(status.value for status in statuses)!r}"
                            )
            ...
            try:
                ...
                if rollout_ids is not None:
                    assert self._recovery_ledger is not None
                    if recover_existing_group:
                        if isinstance(error, _FinalizedGroupDropped):
                            recovery_group_id = None
                    elif isinstance(error, _FinalizedGroupDropped):
                ...
                if not recover_existing_group:
                    try:
                        ...
                        print(
                            f"  warn: remove_group({group_id}) cleanup failed: "
                            f"{cleanup_exc!r}",
                            flush=True,
                        )
                ...
                if rollout_ids is not None and not recover_existing_group:
                    assert self._recovery_ledger is not None
                    self._recovery_ledger.abandon_group(group_id)
                    await self._fail_capture_rollouts(group_id, rollout_ids)
                if not recover_existing_group:
    ...
    async def ensure_rollout_group(
        ...
        *,
        ...
        if statuses == {RolloutAttemptStatus.RESERVED}:
            outcome = await self.generate_and_push(
                input_sample,
                target_step=target_step,
                recovery_group_id=group_id,
            )
            return outcome is RolloutOutcome.COMMITTED
        recoverable_statuses = {
            RolloutAttemptStatus.SEALED,
            RolloutAttemptStatus.ABANDONED,
            RolloutAttemptStatus.FAILED,
    ...
    async def _fail_capture_rollouts(
        ...
        if self._recovery_ledger is not None:
    ...
    async def _recover_group_once(self, group_id: str) -> bool:
        ...
        try:
            ...
            if recovery_group_discarded:
                raise
            try:
                await self._tq_buffer.remove_group(group.group_id)
            except Exception as cleanup_error:
                print(
                    f"warn: remove_group({group.group_id}) recovery cleanup "
